...
string input_soHD(
	...
	writeText(l + kcl, t + kct, result, 3, i_color, f_medium, i_highlight); // chữ trong input
	while (!key_enter) {
		if (ismouseclick(WM_LBUTTONDOWN)) {
			getmouseclick(WM_LBUTTONDOWN, x, y);
			if (!ktVT(l, t, r, b, x, y)) {
				...
				setfillstyle(1, 15);
				bar3d(l, t, r, b, 0, 0);
				writeText(l + kcl, t + kct, result, 3, i_color, f_medium, 15);
				break;
